SELECT
        a.TRANSACTION_ID,
  a.TRANSACTION_DATE,
  a.NAME_PERSONAL_ACCOUNT,
  a.AMOUNT,
  a.ACCOUNT_BALANCE,
  a.TRANSACTION_DESCRIPTION,
  a.BENEFICIARY_IBAN,
  a.FINANZGURU_CATEGORY_LVL1,
  a.FINANZGURU_CATEGORY_LVL2,
  a.IS_CONTRACT,
  a.CONTRACT_RECURRENCE,
  a.CONTRACT_ID,
  a.IS_TRANSFER,
  a.SCOPE_OUT,
  a.TRANSACTION_TYPE,
  a.INCOME_EXPENSE,
  a.PERSONAL_IBAN,
  a.FILENAME,
  a.PROCESSING_DATE,
  a.TXT_CATEGORY_LVL3,
  a.RECEIVER_IS_PERSONAL_ACCOUNT_FLAG,
  a.BENEFICIARY,
a.beneficiary_iban_masked,
a.pair_id,
a.is_self_cancelling_transaction,
a.accounts_are_both_internal_movements,
a.beneficiary_account_preferred_category,
    CASE WHEN b.TXT_CATEGORY_LVL1 = 'Income' THEN a.AMOUNT ELSE - a.AMOUNT END AS SPEND
FROM {{ ref('fact_with_corrections') }} a
LEFT JOIN {{ref('categorytree')}} b ON a.TXT_CATEGORY_LVL3 = b.TXT_CATEGORY_LVL3
